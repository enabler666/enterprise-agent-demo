

## 关于LangGraph的使用
2026-07-14 10:40:00


```python
    def __init__(self, model: Runnable[Any, BaseMessage], tools: RequirementTools) -> None:
        self._model = model
        self._tools = tools
        builder = StateGraph(RequirementAgentState)
        # 节点只交换 State 的增量：messages 由 State 中的 reducer 追加，tool_rounds 用于限制
        # “模型请求工具 → 工具结果回注模型”的循环次数，避免异常模型响应造成无限调用。
        builder.add_node("model", self._call_model)
        builder.add_node("tools", self._execute_tools)
        builder.add_edge(START, "model")
        builder.add_conditional_edges("model", self._route_after_model, {"tools": "tools", "end": END})
        builder.add_edge("tools", "model")
        # LangGraph 必须 compile 后才可调用；编译会检查孤立节点和边连接。
        self._graph = builder.compile()
        # graph的可行一条路径 model(最开始注册模型时提供了tool) -> 语言模型返回了标准的json，表示需要一个工具调用 -> tools节点 -> model节点(查询信息放到了上下文,再次模型推理) -> end
```
这段代码组成的有向无环图其实是这样的:
```
START
  ↓
model
  │
  │（DeepSeek 已经知道有哪些 Tool 可以调用）
  │
  ├── 返回普通 AIMessage
  │        ↓
  │       END
  │
  └── 返回 AIMessage(tool_calls)
           ↓
        tools
           │
           │（Python Tool → Java API）
           │
           │生成 ToolMessage
           │（写回 State.messages）
           ↓
         model
           │
           │（模型读取 ToolMessage）
           │
      返回最终 AIMessage
           ↓
          END
```

模型的graph会共享上下文，当模型发现需要调用预先生命的工具时，会将工具调用结果写回上下文，我应该在代码中执行tool并把结果放到上下文，接下来继续对话。



## 关于为什么不ALl in one到Java程序或Python程序
2026-07-14 10:49:00
我认为的，大多数场景，企业已有Java服务或GO作为传统的后端，所以需要补个Python Agent来调用Java服务或GO服务。
至于为什么不直接在Java中实现，如果轻量级的内容我觉得这样做更少，还减少了维护成本
但是实际上Python对这玩意支持更好，而且可以更好的做业务拆分，从管理上来说算是两个岗位，从技术上来说业务隔离更为明显，Python服务只需要关注Java端暴露的API，可以更好的做权限控制，至少不会出现AI服务可以直接调用CRUD方法的情况。
